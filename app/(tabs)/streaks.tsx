...
export default function SignInScreen() {
  ...
  const { data: completedHabits = [], isLoading: completedHabisLoading } =
    useCompletedHabit(userId, false);
  ...
  const getStreakData = (habitId: string) => {
    const habitCompletions = [...completedHabits]
      .filter((h) => String(h?.habit_id) === String(habitId))
      .sort(
        (a, b) =>
    ...
    if (habitCompletions.length === 0)
      return { streak: 0, bestStreak: 0, total: 0 }; // unified return shape
      ...
      {rankedHabits.length > 3 && (
        <View style={styles.rankingContainer}>
          ...
          {rankedHabits.slice(0, 3).map((item, key) => (
            <View key={`ranked-${key}`} style={styles.rankingRow}>
